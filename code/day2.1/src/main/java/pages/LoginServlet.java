		...
		try (PrintWriter pw = response.getWriter()) {
			...
			if (user == null) // login failed
				pw.print("<h4> Invalid Email or Password , Please <a href='login.html'>Retry</a></h4>");
					...
					// create a cookie obj
					// javax.servlet.http.Cookie(nm,val)
				Cookie c1 = new Cookie("user_det", user.toString());
				// add to resp header
				// method of httpservresp:public void addCookie
				response.addCookie(c1);// no of times req
				...
				if (user.isVotingStatus()) {// alrdy voted
					...
					// WC clears(i.e. empties )resp buffer
					// WC sends temp redirect resp : SC 302 | location=candidate_list,
					//Set-Cookie: user_det :userdetails.toString() | body : EMPTY
					// web browser : sends NEXT request : http://host:port/day2.1/candidate_list,
					//Check prvacy settings--if blocked--nothing
					//if unblock--chk cookie age(expiry)=>(-1)--store cookie in cache
					//Send next request : http://host:port/day2.1/candidate_list
					// method =GET+req_header:cookie--user_det:userdet.toString()
